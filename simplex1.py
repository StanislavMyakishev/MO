...
def maxz(table, output='summary'):
    ...
    while next_round_r(table)==True:
        table = pivot(loc_piv_r(table)[0],loc_piv_r(table)[0],table)

    while next_round(table)==True:
        table = pivot(loc_piv(table)[0],loc_piv(table)[0],table)
...
def quantitySum(quan, costs):
    ...
    for key in quan:
        quanAr.append(int(quan[key]))

    for key in costs:
        costsAr.append(int(key))

    while (i < len(quanAr)):
        summary = summary + quanAr[i] * costsAr[i]
        i = i + 1
...
if __name__ == "__main__":
    ...
    m = gen_matrix(4,3)
    constrain(m,'6,3,1,4,L,252')
    constrain(m,'2,4,5,1,L,144')
    constrain(m,'1,2,4,3,L,80')
    obj(m, costs)
    print(quantitySum(maxz(m, 'summary'), costs.split(',')))